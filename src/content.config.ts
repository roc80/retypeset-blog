...
const posts = defineCollection({
  loader: glob({ pattern: ['**/*.{md,mdx}', '!weeks/**/*'], base: './src/content/posts' }),
  schema: z.object({
    // required
    title: z.string(),
    pubDate: z.date(),
...
const weeks = defineCollection({
  loader: glob({ pattern: '**/*.{md,mdx}', base: './src/content/posts/weeks' }),
  schema: z.object({
    // required
    title: z.string(),
    pubDate: z.date(),
    ...
    tags: z.array(z.string()).optional().default(['周记']),
    // Advanced
    draft: z.boolean().optional().default(false),
    pin: z.number().int().min(0).max(99).optional().default(0),
    toc: z.boolean().optional().default(themeConfig.global.toc),
